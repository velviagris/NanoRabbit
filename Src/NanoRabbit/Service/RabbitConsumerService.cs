...
{
    public class RabbitConsumerService<TConfiguration> : BackgroundService
    where TConfiguration : RabbitConfiguration
    {
        private readonly ILogger<RabbitConsumerService<TConfiguration>> _logger;
        ...
        public RabbitConsumerService(
            ILogger<RabbitConsumerService<TConfiguration>> logger,
            ...
            IServiceProvider serviceProvider)
        {
            ...
            _instanceId = $"{_options.ConsumerName}-{Guid.NewGuid().ToString("N")[..6]}"; // 创建一个简短的唯一标识符
        ...
        private bool HandleMessageReceived(BasicDeliverEventArgs ea, CancellationToken stoppingToken)
        {
            ...
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                try
                {
                    ...
                    processedSuccessfully =
                        messageHandler.HandleMessage(messageBody, ea.RoutingKey, correlationId);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex,
